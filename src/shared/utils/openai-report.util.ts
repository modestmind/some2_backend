...
+ 리포트 구성 (총 8개 섹션으로 나뉨)
	...
	6) 썸 손절 판별 (최종 결론)
		- 두 사람의 궁합 점수를 100점 만점 기준으로 아래처럼 표시한다. (아래 형식 준수)
=========================
...
[중요] 각 섹션 내용을 작성할때 문맥상 중요한 단어나 문장에 밑줄, 폰트색상, 텍스트배경색, 박스영역 등의 스타일을 적용해줘. 각 섹션마다 최소한 3곳 이상은 적용해줘. p태그로 단락 구분을 하고 단락 구분 p태그에는 아무런 스타일도 적용하지 말아줘. ** 같은 마크다운 문법은 사용하지 말고, html 태그나 인라인 css 스타일을 적용해서 각 섹션의 내용을 html로 만들어줘.
`;
